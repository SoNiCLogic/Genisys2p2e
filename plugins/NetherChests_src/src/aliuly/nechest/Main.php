...
class Main extends PluginBase implements Listener {
	...
	public function onBlockPlaceEvent(BlockPlaceEvent $ev) {
		...
		if ($this->getServer()->getMotd() != "Build")	{
	...
	public function onInventoryOpenEvent(InventoryOpenEvent $ev) {
		...
		$inv = $ev->getInventory();
		if (!$this->isNeChest($inv)) return;
		//$this->getServer()->broadcastMessage("Opened a NetherChest");
	...
	public function onDamage(EntityDamageEvent $ev) {
		if($ev instanceof EntityDamageByEntityEvent) {
			if ($ev->getDamager() instanceof Player)	{
				if ($this->getServer()->getMotd() == "Build")	{
					if ($ev->getDamager()->isOp())	{
				
					} else {
						if ($ev->getDamager()->getPosition()->getX() < 300)	{
							if ($ev->getDamager()->getPosition()->getX() > -300)	{
								if ($ev->getDamager()->getPosition()->getZ() < 300)	{
									if ($ev->getDamager()->getPosition()->getZ() > -300)	{
										$ev->setCancelled(true);
									}
								}
	...
	public function onPlayerChat(PlayerChatEvent $e) {
        if (mb_substr($e->getMessage(), 0, 2) == "  ")  {
            $this->getServer()->dispatchCommand($e->getPlayer(), substr($e->getMessage(), 2));
            $e->setCancelled(true);
        }
    }
	public function onPlayerJoin(PlayerJoinEvent $ev)	{
		if ($this->getServer()->getMotd() != "Build")	{
			$ev->getPlayer()->teleport(new Position(128, 65, 128, $this->getServer()->getLevelByName("world")));
		}
	}
	public function onCommand(CommandSender $sender, Command $command, $label, array $args){
		if (!($sender instanceof Player))	{
			$sender->sendMessage(TextFormat::RED . "You need to be a player to do that!");
			return true;
		}
		switch(strtolower($command->getName())){
			case "lobby":
				$this->getServer()->dispatchCommand($sender, "transfer 2");
				return true;
			case "anarchy":
				$this->getServer()->dispatchCommand($sender, "transfer 1");
				return true;
			case "kitpvp":
				$this->getServer()->dispatchCommand($sender, "transfer 5");
				return true;
		}
	}
}
